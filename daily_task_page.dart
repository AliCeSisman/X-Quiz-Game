...
class DailyTaskPage extends StatefulWidget {
  ...
  const DailyTaskPage({super.key, required this.userId});
...
class _DailyTaskPageState extends State<DailyTaskPage> {
  ...
  int correctCount = 0;
  int wrongCount = 0;
  int skippedCount = 0; // Soru atlama sayısı
  Map<String, int> wrongCategories = {}; // Kategori adı -> yanlış sayısı
  ...
  void initState() {
    super.initState();
    checkDailyTaskAvailability();
  }

  Future<void> checkDailyTaskAvailability() async {
    try {
      ...
      if (doc.exists) {
        // Firestore'dan "lastPlayed" alanını al ve DateTime'a dönüştür
        final Timestamp? lastPlayedTimestamp = doc.data()?['lastPlayed'];
        if (lastPlayedTimestamp != null) {
          final lastPlayed =
              lastPlayedTimestamp.toDate(); // Timestamp'ten DateTime'a çevir

          // Şu anki zaman ile karşılaştır
          final difference = DateTime.now().difference(lastPlayed);
          final remainingMinutes =
              (12 * 60) - difference.inMinutes; // 12 saat -> 720 dakika

          if (remainingMinutes > 0) {
            final remainingHours = remainingMinutes ~/ 60;
            final remainingMins = remainingMinutes % 60;
            _showCooldownMessage(remainingHours, remainingMins);
            return;
          }
        }
      }

      // Eğer zaman dolmuşsa veya hiç oynanmamışsa
      loadQuestions();
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Zaman kontrolü sırasında hata oluştu: $e")),
      );
    }
  }

  void _showCooldownMessage(int remainingHours, int remainingMins) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
            "Günlük görev zaten oynandı. Tekrar oynamak için $remainingHours saat, $remainingMins dakika bekleyin."),
      ),
    );
  ...
  Future<void> loadQuestions() async {
    try {
      final hardQuestionsSnapshot =
          await FirebaseFirestore.instance.collection('hard_questions').get();
      final easyQuestionsSnapshot =
          await FirebaseFirestore.instance.collection('questions').get();

      final hardQuestions = hardQuestionsSnapshot.docs
          .map((doc) => doc.data())
          .toList()
        ..shuffle();
      final easyQuestions = easyQuestionsSnapshot.docs
          .map((doc) => doc.data())
          .toList()
        ..shuffle();
      ...
      setState(() {
        questions = [
          ...hardQuestions.take(8),
          ...easyQuestions.take(7),
        ];
      });
      startTimer();
    } catch (e) {
  ...
  void finishGame() async {
    ...
    await FirebaseFirestore.instance
        .collection('daily_tasks')
        .doc(widget.userId)
        .set({'lastPlayed': Timestamp.now()}); // Son oynanma zamanını kaydet

    await updateUserStats(); // Kullanıcı istatistiklerini güncelle

    // Oyun bitim ekranına git
